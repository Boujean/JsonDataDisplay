...
import { ToggleDivM, ToggleDivMD, ToggleDivWM } from '../Toggle_Show_button.js'
        ...
        <ToggleDivM>
          <div className='MonthsContainer'>
            {Disp_data.Years_Features.map(function (months, index) {
              return <div className='Months' key={index.toString()}>
                {months.Months_Features.map(function (month_details, i) {
                  return <div className='Month' key={i.toString()}>
                      ...
                      <ToggleDivWM>
                        <div className='WeekData'>
                          ...
                          {month_details.Weeks_Features.map(function (weeks_details, l) {
                            return <li key={i + l} className='map_result'> Week {month_details.Weeks[l]} : {weeks_details.Correct.toString()}</li>
                          })}
                          <p className='Titlesh3'> For an average duration in minutes </p>

                          {month_details.Weeks_Features.map(function (weeks_details, l) {
                            return <li key={i + l} className='map_result'>Week {month_details.Weeks[l]} : {weeks_details.Correct_Duration.toString()}</li>
                          })}
                          <p className='Titlesh3'> people who parked longer than the allowed time </p>
                          {month_details.Weeks_Features.map(function (weeks_details, l) {
                            return <li key={i + l} className='map_result'>Week {month_details.Weeks[l]} : {weeks_details.Violators.toString()}</li>
                          })}
                          <p className='Titlesh3'> For an average duration in minutes </p>
                          {month_details.Weeks_Features.map(function (weeks_details, l) {
                            return <li key={i + l} className='map_result'>Week {month_details.Weeks[l]} : {weeks_details.Violators_Duration.toString()}</li>
                          })}
                        </div>
                      </ToggleDivWM>
                    </div>
